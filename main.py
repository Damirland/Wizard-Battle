...
class Enemy(pg.sprite.Sprite):
    ...
    def handle_attack_mode(self, player):
        if not self.attack_mode:
            ...
            if random.randint(1, 300) <= attack_chance:
...
class Game:
    def __init__(self, mode, wizards):
        ...
        self.grass = load_image("images/grass.png", SCREEN_WIDTH, SCREEN_HEIGHT)
    ...
    def draw(self):
        ...
        pg.draw.rect(self.screen, pg.Color("black"), (SCREEN_WIDTH - 210, 10, 200, 20), 2)

        if self.win == self.player:
            text = text_render("ПОБЕДА")
            text_rect = text.get_rect(center=(SCREEN_WIDTH // 2, SCREEN_HEIGHT // 2))
            self.screen.blit(text, text_rect)
            text2 = text_render("Левый маг")
